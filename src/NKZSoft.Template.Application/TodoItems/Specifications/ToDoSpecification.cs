...
public sealed class ToDoItemSpecification : Specification<ToDoItem>
{
    ...
    private ISpecificationBuilder<ToDoItem> Sort(ISpecificationBuilder<ToDoItem> specificationBuilder, IEnumerable<SortDescriptor> sorts) =>
        sorts.Aggregate(specificationBuilder, Sort);

    private ISpecificationBuilder<ToDoItem> Sort(ISpecificationBuilder<ToDoItem> specificationBuilder, SortDescriptor sort) =>  
            SortExpressions.TryGetValue(sort.Field, out var se)
            ? (sort.Direction == EnumSortDirection.Desc
